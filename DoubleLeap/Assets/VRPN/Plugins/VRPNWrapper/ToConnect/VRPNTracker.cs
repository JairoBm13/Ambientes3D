...
public class VRPNTracker : MonoBehaviour {
	...
	//M¨¦todo que se encarga de estar pendiente de los mensajes 
	//que se env¨ªan desde los sensores an¨¢logos del dispositivo
	void moverConRaton(string name, VRPNTracker.TrackerReport report)
	{
		//Se debe notar que los valores que se reportan son de 0 a 1, 
		//siendo (0, 0) la esquina superior izquierda de la pantalla 
		//y (1, 1) la esquina inferior derecha de la pantalla
		this.transform.position = 
			new Vector3((float)report.pos[0], 
				(1-(float)report.pos[1])*10 - 5, 
				(float)report.pos[2]);
	}
}